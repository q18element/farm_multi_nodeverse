...
export default class HahaWallet extends BaseService {
    ...
    async getOtpBiz(filter) {
        ...
        try {
            ...
            for (let i = 0; i < emails.length; i++) {
                ...
                if ((!filter.subject || email.subject.includes(filter.subject)) &&
        ...
        try {
            ...
            if (!isLogin) {
                ...
                for (const email of emails) {
                    if ((!filter.subject || email.su.includes(filter.subject)) &&
    ...
    async getVerifyCode(fromTime = 0) {
        ...
        let domain = username.split("@").pop(), otp = null;
        var filter = {
        ...
        if (!(await isLoggedIn())) {
            ...
            await driver.executeScript(() => {
                document.querySelector('input[placeholder="Referral Code"]')
                    ? null // @ts-ignore
                    : document
                        .querySelector("#app-content > div > div.w-\\[450px\\].max-h-\\[600px\\].bg-white.text-black.dark\\:bg-dark.dark\\:text-white.rounded-lg.overflow-hidden.relative > div.flex.flex-col.p-10 > div.flex.flex-row.items-center.gap-2.cursor-pointer") // @ts-ignore
                        .click();
            });
            await driver.sleep(500);
