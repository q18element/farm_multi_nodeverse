...
async function fillRecoveryInputsWithClickAndSendKeys(driver: WebDriver, recoveryKeyArray: string | string[]) {
  ...
  try {
    ...
    for (let inputElement of inputElements) {
      const testid: string = await driver.executeScript("return arguments[0].dataset.testid;", inputElement);
...
async function fillImportSrpRecoveryWords(driver: WebDriver, recoveryKeyArray: string | string[]) {
  if (typeof recoveryKeyArray === "string") {
    recoveryKeyArray = recoveryKeyArray.split(" ");
  }
  try {
