...
export default class TaskDB {
  ...
  async getPendingAutomations(profileId) {
    const rows = await this.db.all(`SELECT service FROM task_monitoring WHERE profile_id = ? AND state != 'failed'`, [
      profileId,
    ]);
  ...
  async updateTaskState(accountId, proxy, service, state, retryIncrement = 0, point = 0) {
    ...
    logger.info(
      `Updated task state for account ${accountId} on proxy ${proxy}: ${service} -> ${state}, point: ${point}`
    );
  }
}
